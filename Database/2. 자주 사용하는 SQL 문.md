...
SELECT 문에서 선택할 컬럼이 여러개이면 ","를 통해서 구별하며 전체 컬럼을 원하면 "*"를 사용합니다.
...
IN 연산자는 특정 컬럼의 값이 주어진 값 목록 중 하나와 일치하는 경우 해당 컬럼의 데이터를 가져오는데 사용되는 연산자 입니다.
...
LIKE 연산자는 특정 패턴과 일치하는 문자열을 검색할 때 사용되는 연산자 입니다.
...
COUNT 함수는 특정 컬럼의 값들의 갯수를 카운트할 때 사용하는 함수입니다.
...
MIN 함수는 특정 컬럼의 값들 중에서 최솟값을 찾을 때 사용하는 함수입니다.
...
MAX함수는 특정 컬럼의 값들 중에서 최댓값을 찾을 때 사용하는 함수입니다.
